import { motion } from "framer-motion"
import { waLink } from "@/data/site"

function WhatsAppIcon(props: React.SVGProps<SVGSVGElement>) {
  return (
    <svg viewBox="0 0 24 24" fill="currentColor" {...props}>
      <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.626.712.226 1.36.194 1.872.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347" />
      <path d="M12.031 21.785h-.006c-1.771-.001-3.5-.451-5.03-1.303L2 21.988l1.542-4.99a10.02 10.02 0 0 1-1.394-5.06C2.15 6.442 6.596 2 12.032 2c2.638 0 5.117 1.03 6.984 2.9a9.822 9.822 0 0 1 2.9 6.99c-.003 5.437-4.449 9.895-9.885 9.895m5.83-15.71a8.288 8.288 0 0 0-5.83-2.42c-4.556 0-8.264 3.706-8.266 8.26a8.222 8.222 0 0 0 1.294 4.421l-.176.28-.85 3.104 3.183-.834.269.16a8.234 8.234 0 0 0 4.19 1.148h.004c4.554 0 8.263-3.708 8.266-8.263a8.213 8.213 0 0 0-2.084-5.856" />
    </svg>
  )
}

export function WhatsAppFloat() {
  return (
    <motion.a
      href={waLink("Hi Lapsoo, I have a question about your laptops.")}
      target="_blank"
      rel="noopener noreferrer"
      initial={{ opacity: 0, scale: 0.6 }}
      animate={{ opacity: 1, scale: 1 }}
      transition={{ delay: 1, duration: 0.4, ease: [0.16, 1, 0.3, 1] }}
      whileHover={{ scale: 1.08 }}
      whileTap={{ scale: 0.95 }}
      className="fixed bottom-6 right-6 z-50 flex h-14 w-14 items-center justify-center rounded-full bg-[#25D366] text-white shadow-[0_10px_30px_-8px_rgba(37,211,102,0.7)]"
      aria-label="Chat on WhatsApp"
    >
      <WhatsAppIcon className="h-7 w-7" />
      <span className="absolute inset-0 rounded-full bg-[#25D366] animate-ping opacity-30" />
    </motion.a>
  )
}
