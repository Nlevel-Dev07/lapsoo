...
const rowSchema = z.object({
  ...
  priceFrom: z.coerce.number().int().nonnegative().catch(0),
  processor: z.string().min(1),
  ram: z.string().min(1),
  storage: z.string().min(1),
  display: z.string().catch(""),
  graphics: z.string().catch(""),
  battery: z.string().catch(""),
  warranty: z.string().min(1),
  screenSize: z.coerce.number().nonnegative().catch(0),
